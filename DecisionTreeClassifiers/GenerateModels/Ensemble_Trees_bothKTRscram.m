...
% models for classification of p38 and NFkB activity trajectories, with p38 cell IDs scrambled, to stimulus
% identity and dose
...
[ModelBothKTRscram.(name), validationAccuracyBothKTRscram.(name),validationPredictionsBothKTRscram.(name), validationScoresBothKTRscram.(name), validationAccuracyBothKTRscram2.(name), StatsValidationBothKTRscram.(name)] = Tree_Ensemble_Classification(trainingCondition,name, [1:4]');

%% PAMPs only, 3 Stim, Dose 6
trainingCondition = 'CC_input_AllLig_Rep1_Dose6_both';
name = 'PAMPsHighDosePool';
[ModelBothKTRscram.(name), validationAccuracyBothKTRscram.(name),validationPredictionsBothKTRscram.(name), validationScoresBothKTRscram.(name), validationAccuracyBothKTRscram2.(name), StatsValidationBothKTRscram.(name)] = Tree_Ensemble_Classification(trainingCondition,name, [1,3,4]');
...
name = 'LPS_Doses_Pool';
[ModelBothKTRscram.(name), validationAccuracyBothKTRscram.(name),validationPredictionsBothKTRscram.(name), validationScoresBothKTRscram.(name), validationAccuracyBothKTRscram2.(name), StatsValidationBothKTRscram.(name)] = Tree_Ensemble_Classification(trainingCondition, name, DosesToClassify');
...
% load input data (csv file, can be found here % Input data availabe as csv here:
% in this repository: 'CombinatorialSignalingMacrophages_p38NFkB\DecisionTreeClassifiers\Inputs_2PoolRep_DecTrees' 
...
% Uniform random selection of cells
    ...
    %scrambling KTR cell IDs (keep set of features per cell together): every uneven column in trainingDataZscored_EqualCell (except column 1 is KTR metric) 
    shuffling_index = randperm(size(trainingDataZscored_EqualCell, 1), size(trainingDataZscored_EqualCell, 1));
    trainingDataZscored_EqualCell(:,3:2:end) = trainingDataZscored_EqualCell(shuffling_index, 3:2:end);
...
title(['KTRscrambled', name])

% saveas(gcf, [OneDrivePath, '\PostDoc UCLA\1 Post Doc UCLA\Matlab analysis\Classifiers\Z Score\PooledReps\BothKTRscram\',name,'_5fVal_EqualCell' '.fig'])
% saveas(gcf, [OneDrivePath, '\PostDoc UCLA\1 Post Doc UCLA\Matlab analysis\Classifiers\Z Score\PooledReps\BothKTRscram\',name,'_5fVal_EqualCell' '.svg'])
