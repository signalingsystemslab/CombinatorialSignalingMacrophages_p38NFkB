...
% models for classification of p38 and NFkB activity trajectories to stimulus
% identity and dose
...
[ModelBoth.(name), validationAccuracyBoth.(name),validationPredictionsBoth.(name), validationScoresBoth.(name), validationAccuracyBoth2.(name), StatsValidationBoth.(name)] = Tree_Ensemble_Classification(trainingCondition,name, [1:4]');

%% PAMPs only, 3 Stim, Dose 6
trainingCondition = 'CC_input_AllLig_Rep1_Dose6_both';
name = 'PAMPsHighDosePool';
[ModelBoth.(name), validationAccuracyBoth.(name),validationPredictionsBoth.(name), validationScoresBoth.(name), validationAccuracyBoth2.(name), StatsValidationBoth.(name)] = Tree_Ensemble_Classification(trainingCondition,name, [1,3,4]');
...
name = 'LPS_Doses_Pool';
[ModelBoth.(name), validationAccuracyBoth.(name),validationPredictionsBoth.(name), validationScoresBoth.(name), validationAccuracyBoth2.(name), StatsValidationBoth.(name)] = Tree_Ensemble_Classification(trainingCondition, name, DosesToClassify');
...
% load input data (csv file, can be found here in this repository: 'CombinatorialSignalingMacrophages_p38NFkB\DecisionTreeClassifiers\Inputs_2PoolRep_DecTrees')

% read in training data table from path as appropriate
...
%saveas(gcf, [OneDrivePath, '\PostDoc UCLA\1 Post Doc UCLA\Matlab analysis\Classifiers\Z Score\PooledReps\BOTHmetrics\',name,'_5fVal_EqualCell' '.fig'])
%saveas(gcf, [OneDrivePath, '\PostDoc UCLA\1 Post Doc UCLA\Matlab analysis\Classifiers\Z Score\PooledReps\BOTHmetrics\',name,'_5fVal_EqualCell' '.svg'])
